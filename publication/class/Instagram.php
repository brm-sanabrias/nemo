...
class Instagram
{
     ...
     * @throws \MetzWeb\Instagram\InstagramException
     */
    public function getLoginUrl($scopes = array('basic', 'likes', 'comments', 'relationships','public_content'))
    {
     ...
     * @return mixed
     */
    public function searchTags($name,$codea)
    {
        return $this->_makeCall('tags/search', false, array('q' => $name,'access_token'=>$codea));
    }
    /**
     ...
     * @return mixed
     */
    public function getTagMedia($name, $limit = 0,$codea)
    {
        $params = array();
        if ($limit > 0) {
            $params['count'] = $limit;
        }
        $params['access_token'] = $codea;
        print_r($params);
    ...
    public function pagination($obj, $limit = 0)
    {
        if (is_object($obj) && !is_null($obj->pagination)) {
            ...
            if (isset($obj->pagination->next_max_id)) {
                return $this->_makeCall($function, $auth, array('max_tag_id' => $obj->pagination->next_max_id, 'count' => $limit));
            }
